...
def _derive_standard_normal(
    ...
    # Normalise so that applying the mapping to the full centred input yields
    # std ≈ 1. Using only the decimated fitting sample can drift on heavy-tail
    # inputs and break downstream expectations.
    ...
    im_std = np.interp(im, x_interp, y_interp).std()
...
def _extended_interp_bins(
    ...
    y_left = y[0] - 2.0 * y_step_left
    y_right = y[-1] + 2.0 * y_step_right
