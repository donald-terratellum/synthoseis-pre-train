...
class SeismicUNet3d(nn.Module):
    ...
    def __init__(
        ...
        block_type: str = "resblock",
        use_checkpoint: bool = True,
        pre_head_mode: str = "identity",  # "norm_gelu" suppresses negative amplitudes — do not use for reconstruction
    ):
        super().__init__()
...
def create_model(
    ...
    **kwargs,
    ...
    """
    # Keep factory resilient to train/model version skew by dropping unknown
    # kwargs (e.g., when train.py passes a newer arg but models.py is older).
    ctor_params = set(inspect.signature(SeismicUNet3d.__init__).parameters)
    ctor_params.discard("self")
    filtered_kwargs = {k: v for k, v in kwargs.items() if k in ctor_params}
    dropped_kwargs = sorted(k for k in kwargs if k not in ctor_params)
    if dropped_kwargs:
        print(f"WARNING: create_model dropped unsupported kwargs due to version skew: {', '.join(dropped_kwargs)}")
